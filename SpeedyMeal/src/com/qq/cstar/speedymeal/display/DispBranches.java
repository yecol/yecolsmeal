...
public class DispBranches {
	...
	public static void display(HttpServletResponse response,ArrayList<Branch> branches){
		...
		out
				.print("<div class='gTitle'> 分店管理 </div><div id='branchList' class='sidebar_block'><p> <a href='MerchantNavi!branch.action' class='n'>添加</a>新的分店。或者<a href='MerchantNavi!manage.action' class='n'>返回</a>管理页。 </p>");
		if (branches.size() != 0) {
			for (int index = 0; index < branches.size(); index++) {
				...
				out
						.print("<div class='ls_h_opr'><a href='javascript:ajaxLoad(\"MerchantAction!delBranch.action?bid="+branches.get(index).getBid()+"\");'"+"class='mer_del_a' id='del"+index+"'>删除</a><a href='index.jsp' class='opr_a'>更新</a></div><div class='clear'></div></div>");
